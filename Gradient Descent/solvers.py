...
#References : https://sebastianraschka.com/faq/docs/sgd-methods.html, https://www.easy-tensorflow.com/tf-tutorials/basics/graph-and-session
...
class GradientDescentSolver(Solver):
    ...
    def get_updates_without_momentum(self, loss_tensor, param_vars):
        ...
        print(type(grad_tensors))
        print(type(loss_tensor))
        ...
        for param_var in param_vars:
            list1.append(param_vars)
            
        print(list1)   
        
        for grad_tensor   in grad_tensors:
            list2.append(grad_tensor)
            
        print(list2) 
        i =0
        for param_var in param_vars:
            product = tf.math.scalar_mul(alpha, list2[i]) #Multiplies a scalar times a Tensor or IndexedSlices object.
            new_param_var = tf.math.subtract(param_var, product)
            list3.extend([(param_var, new_param_var)])
            i= i+1
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        for iter_ in range(self.iterations):
            ...
            train_loss, U_value = session.run([loss_tensor,update_ops], feed_dict = training_dict)
            #print(train_loss)
            #print(U_value)
...
class StochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        batch_size = 1
        train_sample_generator = MinibatchIndefinitelyGenerator(train_data, batch_size, self.shuffle) 
        test_sample_generator = MinibatchIndefinitelyGenerator(val_data, batch_size , self.shuffle)
        ...
        for iter_ in xrange(self.iterations):
            "*** YOUR CODE HERE ***"
            #util.raiseNotDefined()
            training_data = train_sample_generator.next()
        
            training_dict = {}
            i =0
            for placeholder in placeholders:
                training_dict[placeholder] = training_data[i]
                i = i + 1
            ...
            testing_data = test_sample_generator.next()
            validation_dict = {}
            j =0
            for placeholder in placeholders:
                validation_dict[placeholder] = testing_data[j]
                j = j + 1
            val_loss = session.run(loss_tensor, feed_dict = validation_dict)
...
class MinibatchStochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        for iter_ in range(self.iterations):
            ...
            training_data = trainGenerator.next()
            training_dict = {}
            i =0
            for placeholder in placeholders:
                training_dict[placeholder] = training_data[i]
            ...
            testing_data = valGenerator.next()
            validation_dict = {}
            j =0
            for placeholder in placeholders:
                validation_dict[placeholder] = testing_data[j]
